...
using namespace ribble::backend;

static KeyModifiers sdl_mod_to_ribble(SDL_Keymod mod) {
    KeyModifiers out = KeyModifiers::None;
    if (mod & SDL_KMOD_SHIFT)   out = out | KeyModifiers::Shift;
    if (mod & SDL_KMOD_CTRL)    out = out | KeyModifiers::Control;
    if (mod & SDL_KMOD_ALT)     out = out | KeyModifiers::Alt;
    if (mod & SDL_KMOD_GUI)     out = out | KeyModifiers::Super;
    if (mod & SDL_KMOD_CAPS)    out = out | KeyModifiers::CapsLock;
    if (mod & SDL_KMOD_NUM)     out = out | KeyModifiers::NumLock;
    return out;
}
...
static KeyboardKey sdl_keycode_to_ribble(SDL_Keycode key) {
    switch (key) {
        ...
        case SDLK_APOSTROPHE:   return KeyboardKey::Apostrophe;
        ...
        case SDLK_0:            return KeyboardKey::Num0;
        case SDLK_1:            return KeyboardKey::Num1;
        case SDLK_2:            return KeyboardKey::Num2;
        case SDLK_3:            return KeyboardKey::Num3;
        case SDLK_4:            return KeyboardKey::Num4;
        case SDLK_5:            return KeyboardKey::Num5;
        case SDLK_6:            return KeyboardKey::Num6;
        case SDLK_7:            return KeyboardKey::Num7;
        case SDLK_8:            return KeyboardKey::Num8;
        case SDLK_9:            return KeyboardKey::Num9;
        case SDLK_SEMICOLON:    return KeyboardKey::Semicolon;
        case SDLK_EQUALS:       return KeyboardKey::Equal;
        ...
        case SDLK_GRAVE:        return KeyboardKey::GraveAccent;
        ...
        case SDLK_KP_0:         return KeyboardKey::Kp0;
        case SDLK_KP_1:         return KeyboardKey::Kp1;
        case SDLK_KP_2:         return KeyboardKey::Kp2;
        case SDLK_KP_3:         return KeyboardKey::Kp3;
        case SDLK_KP_4:         return KeyboardKey::Kp4;
        case SDLK_KP_5:         return KeyboardKey::Kp5;
        case SDLK_KP_6:         return KeyboardKey::Kp6;
        case SDLK_KP_7:         return KeyboardKey::Kp7;
        case SDLK_KP_8:         return KeyboardKey::Kp8;
        case SDLK_KP_9:         return KeyboardKey::Kp9;
        case SDLK_KP_PERIOD:    return KeyboardKey::KpDecimal;
        case SDLK_KP_DIVIDE:    return KeyboardKey::KpDivide;
        case SDLK_KP_MULTIPLY:  return KeyboardKey::KpMultiply;
        case SDLK_KP_MINUS:     return KeyboardKey::KpSubtract;
        case SDLK_KP_PLUS:      return KeyboardKey::KpAdd;
        case SDLK_KP_ENTER:     return KeyboardKey::KpEnter;
        case SDLK_KP_EQUALS:    return KeyboardKey::KpEqual;
        ...
        case SDLK_MENU:         return KeyboardKey::Menu;
...
Result<void, WindowBackend::Failure> SDLWindow::initialize(int width, int height, const char *title) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO) < 0) return Fail(RIBBLE_ERROR(WindowBackend::Failure::InitializationFailure, "Failed to initialize SDL3 window."));
...
Result<void, WindowBackend::Failure> SDLWindow::poll_events() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED: {
            ...
            m_windowEventBus->dispatch_immediate(
                std::make_shared<WindowContentScaleEvent>(scale, scale));
            break;
        }
        ...
        case SDL_EVENT_TEXT_INPUT: {
            ...
            if      (utf8[0] < 0x80) codepoint = utf8[0];
            else if (utf8[0] < 0xE0) codepoint = ((utf8[0] & 0x1F) << 6)  | (utf8[1] & 0x3F);
            else if (utf8[0] < 0xF0) codepoint = ((utf8[0] & 0x0F) << 12) | ((utf8[1] & 0x3F) << 6) | (utf8[2] & 0x3F);
            else                     codepoint = ((utf8[0] & 0x07) << 18) | ((utf8[1] & 0x3F) << 12) | ((utf8[2] & 0x3F) << 6) | (utf8[3] & 0x3F);
        ...
        case SDL_EVENT_MOUSE_MOTION:
            m_windowEventBus->dispatch_immediate(
                std::make_shared<MouseMoveEvent>(
        ...
        case SDL_EVENT_MOUSE_BUTTON_UP: {
            ...
            m_windowEventBus->dispatch_immediate(
                std::make_shared<MouseButtonEvent>(
                    sdl_mouse_button_to_ribble(event.button.button),
                    action,
        ...
        case SDL_EVENT_GAMEPAD_BUTTON_UP: {
            const ButtonAction action = (event.type == SDL_EVENT_GAMEPAD_BUTTON_DOWN)
                ? ButtonAction::Press : ButtonAction::Release;
            m_windowEventBus->dispatch_immediate(
                std::make_shared<GamepadButtonEvent>(
        ...
        case SDL_EVENT_GAMEPAD_AXIS_MOTION: {
            ...
            m_windowEventBus->dispatch_immediate(
                std::make_shared<GamepadAxisEvent>(
                    static_cast<int>(event.gaxis.which),
                    sdl_gamepad_axis_to_ribble(static_cast<SDL_GamepadAxis>(event.gaxis.axis)),
                    normalized));
            break;
        }
        ...
        case SDL_EVENT_GAMEPAD_ADDED: {
            ...
            m_windowEventBus->dispatch_immediate(
                std::make_shared<GamepadConnectedEvent>(
        ...
        case SDL_EVENT_GAMEPAD_REMOVED:
            m_windowEventBus->dispatch_immediate(
                std::make_shared<GamepadDisconnectedEvent>(
                    static_cast<int>(event.gdevice.which)));
            break;

        case SDL_EVENT_DROP_FILE: {
            std::vector<std::string> paths;
            if (event.drop.data) paths.emplace_back(event.drop.data);
            m_windowEventBus->dispatch_immediate(
...
Result<void, WindowBackend::Failure> SDLWindow::shutdown() {
    ...
    if (!m_window) {
        return Fail(RIBBLE_ERROR(Failure::ShutdownFailure, "Failed to shut down SDL3 window cause it doesnt exist."));
    }
