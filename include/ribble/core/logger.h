 ...
 *
 * Creates logger with rotating files: latest.log, error.latest.log and otherwise <datetime>.log / error.<datetime>.log
 *
...
namespace detail {
    ...
    template<typename FormatString, typename... Args>
    inline std::string FormatMessageWithLocation(FormatString&& fmt, const char* file, size_t line, Args&&... args) {
        std::string formatted = spdlog::fmt_lib::format(fmt::runtime(std::forward<FormatString>(fmt)), std::forward<Args>(args)...);
...
// Info does not include file/line information
    ...
    #define RIBBLE_LOG_DEBUG(fmt, ...) \
        do { \
            ribble::detail::LogDebugSync(ribble::detail::FormatMessageWithLocation(fmt, __FILE__, __LINE__, ##__VA_ARGS__)); \
        } while(0)
    ...
    #define RIBBLE_LOG_WARNING(fmt, ...) \
        do { \
            ribble::detail::LogWarningSync(ribble::detail::FormatMessageWithLocation(fmt, __FILE__, __LINE__, ##__VA_ARGS__)); \
        } while(0)

    #define RIBBLE_LOG_ERROR(fmt, ...) \
        do { \
            ribble::detail::LogErrorSync(ribble::detail::FormatMessageWithLocation(fmt, __FILE__, __LINE__, ##__VA_ARGS__)); \
        } while(0)
#else
